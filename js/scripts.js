var Setings = /** @class */ (function () {
    function Setings() {
        ...
        this.cores = ["AliceBlue", "AntiqueWhite", "Aqua", "Aquamarine", "Azure", "Beige",
        ...
        this.vidas = 4;
        this.i = 0;
        this.corSorteada = this.corAleatoria[Math.floor((Math.random() * this.corAleatoria.length))];
    }
    Setings.prototype.sortear = function (corAleatoria, cores, corSorteada) {
        for (this; this.i < 10; this.i++) {
            corAleatoria.push(cores[Math.floor(Math.random() * cores.length) + 1].toLowerCase());
            corAleatoria.sort;
        }
    };
    Setings.prototype.jogar = function (corSorteada, usuario, vidas) {
        do {
            this.usuario = prompt("ESCOLHA UMA DAS CORES ABAIXO:\n ".concat(this.corAleatoria, "\nVidas Restantes: ").concat(vidas)).toLowerCase();
